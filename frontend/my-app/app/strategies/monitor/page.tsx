            ...
            <CardContent className="h-[350px] w-full flex items-center justify-center">
              {data && mounted ? (
                <ResponsiveContainer width="100%" height="100%" minWidth={1} minHeight={1}>
                  <RadarChart cx="50%" cy="50%" outerRadius="70%" data={data.dimensions}>
              ...
              <table className="w-full text-sm">
                <thead className="bg-slate-800/30">
                  <tr className="border-b border-slate-700/50">
                    ...
                    <th className="text-center py-3 px-4 text-slate-400 font-medium">运行状态</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-slate-800/50">
                  {data?.weights.map((w, idx) => (
                    <tr key={idx} className="hover:bg-slate-800/20 transition-colors">
                      ...
                      <td className="py-3 px-4 text-right font-mono text-slate-300">
                        {((w.value / 100) * (data.totalAllocation || 0)).toLocaleString()} USDT
                      </td>
